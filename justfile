...
initialize:
    ...
    gh repo create Netbeheer-Nederland/dp-capaciteitskaart \
        --description "Information models for the Capaciteitskaart data product." \
        --public \
        --disable-wiki

    git init -b main

    git remote add origin git@github.com:Netbeheer-Nederland/dp-capaciteitskaart.git
    git fetch
    ...
    gh repo edit Netbeheer-Nederland/dp-capaciteitskaart --default-branch v0
    git branch -D main
    ...
    #  -H "Accept: application/vnd.github+json" \
    #  -H "X-GitHub-Api-Version: 2022-11-28" \
    #  /repos/Netbeheer-Nederland/dp-capaciteitskaart/actions/permissions \
    #   -F "enabled=true" -f "allowed_actions=all"

    echo "… OK."
    echo
    echo "A GitHub repository has been created and configured at: https://github.com/Netbeheer-Nederland/dp-capaciteitskaart"
    echo
...
_post-process-linkml-schema:
    ...
    cp "information_models/dp_capaciteitskaart.schema.linkml.yml" "artifacts/information_models/"
    @echo
    @echo "Setting version in LinkML schema…"
    @echo
    sed -i '/^version: .*$/d' "artifacts/information_models/dp_capaciteitskaart.schema.linkml.yml"
    @if [ -z ${VERSION:-} ]; then \
        sed -i "/^name: .*$/a version: {{shell(ref_name)}}" "artifacts/information_models/dp_capaciteitskaart.schema.linkml.yml"; \
    else \
        sed -i "/^name: .*$/a version: ${VERSION}" "artifacts/information_models/dp_capaciteitskaart.schema.linkml.yml"; \
    fi
    @echo "… OK."
    @echo
...
[group("schema")]
edit-schema:
    @${VISUAL:-${EDITOR:-nano}} information_models/dp_capaciteitskaart.schema.linkml.yml

# Show definition of the class identified by the provided CURIE
[group("schema")]
get-definition curie:
    yq '.classes.* | select(.class_uri == "{{curie}}")' information_models/dp_capaciteitskaart.schema.linkml.yml

# List all classes in the schema
[group("schema")]
list-classes:
    yq '.classes.* | key' information_models/dp_capaciteitskaart.schema.linkml.yml
